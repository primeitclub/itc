            ...
            @foreach ($upcomingEvents as $upcomingEvent)
                <div class="flex flex-col overflow-hidden rounded-xl shadow-xl">
                    ...
                    <div class="flex flex-1 flex-col justify-between bg-white p-6">
                        <div class="flex-1">
                            <span
                                class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-primary text-white">{{ $upcomingEvent->eventCategory->title }}</span>
                            <a href="{{ route('events.show', $upcomingEvent->slug) }}" class="mt-2 block">
                                <p class="text-2xl font-semibold text-primary hover:text-primary-light">
                            ...
                            <div class="flex flex-col space-y-2 mt-4">
                                ...
                                <div class="flex space-x-4">
                                    ...
                                    <div class="flex flex-col">
                                        <span
                                            class="font-medium text-medium text-greish">{{ $upcomingEvent->event_date }}</span>
                                        <span class="text-xs text-greish">{{ $upcomingEvent->event_time }} -
                                            Onwards</span>
                                    </div>
                                </div>
        ...
        @foreach ($completedEvents as $completedEvent)
            <div class="hidden lg:flex justify-center items-center ">
                <div class="w-[67%] h-48 bg-white shadow-xl rounded-2xl mb-8 md:mb-24 grid grid-cols-4">
                    <div class="">
                        <img src="{{ asset('/blog-images/groot_1671793404.png') }}" class="rounded-l-2xl h-48 w-full">
                    </div>
                    <div class="flex flex-col col-span-2 pt-4 pl-6 p-4   ">
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <span class="font-medium text-lg text-green-400 pr-4">Free</span>
                            <p class="py-0.5 px-6 text-white rounded-full text-sm bg-primary">
                                {{ $completedEvent->eventCategory->title }}</p>
                        </div>
                        ...
                        <a href="{{ route('events.show', $completedEvent->slug) }}" class="mt-2 block">
                            <button
                                class="text-sm text-gray-500 hover:text-black pt-2 transition-colors duration-150 ease-in-out">Learn
                                more >></button>
                        </a>
                    </div>
                    <div
                ...
                <div class="w-[90%] h-34 bg-white shadow-xl rounded-2xl mb-8 md:mb-24 grid grid-cols-2 relative ">
                    <div>
                        <div class="flex flex-col justify-center col-span-2 pt-4 pl-6 p-2   ">
                            <h1 class="text-primary poppins text-2xl font-bold pb-2">{{ $completedEvent->title }}</h1>
