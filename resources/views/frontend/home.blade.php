<x-frontend.app title="Home">
    ...
    <section class="max-w-7xl flex flex-col-reverse md:flex-row items-center mx-auto px-8 sm:px-6">
            ...
            <button class="button mt-7 py-3 px-6 rounded-3xl">
                <div class="flex justify-between items-center space-x-2">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor"
                        class="w-4 h-4 text-center text-gray-900 border-primary bg-white rounded-full">
        ...
        <div class="mt-4 md:mt-10 -z-10">
            <lottie-player class="robot w-[60%] mx-auto"
                src="https://assets2.lottiefiles.com/packages/lf20_6e0qqtpa.json" background="transparent"
                speed="1.1" loop autoplay>
            </lottie-player>
        </div>
    </section>
                ...
                <div class="flex flex-col max-w-lg">
                    <header class="mt-4 lg:mt-2">
                        <div class="mt-4">
                            ...
                            @endif
                            <span
                                class="px-4 py-2 bg-primary rounded-full text-white text-xs font-semibold">{{ $featuredEvent->eventCategory->title }}</span>
                        </div>
                    </header>
                    ...
                    <div class="flex flex-col space-y-2 mt-4">
                        <div class="flex space-x-4 ">
                            ...
                            <span class="font-medium text-greish">{{ $featuredEvent->venue }},
                                {{ $featuredEvent->venue_address }}</span>
                        </div>
                        ...
                        <div class="flex space-x-4">
                            ...
                            <div class="flex flex-col">
                                <span
                                    class="font-medium text-medium text-greish">{{ $featuredEvent->event_date }}</span>
                        ...
                        <div class="flex space-x-4">
                            ...
                            @if($featuredEvent->event_fee)
                            <span class="font-medium text-medium text-greish">Rs. {{ $featuredEvent->event_fee }}</span>
                            @else
                ...
                <div class="flex flex-col max-w-lg">
                    <header class="mt-4 lg:mt-2">
                        <div class="mt-4">
                            ...
                            @if ($completedFeaturedEvent->speakers)
                                @foreach ($completedFeaturedEvent->speakers as $speaker)
                                    <span class="mt-2 mb-4 block text-greish text-sm lg:text-xl font-semibold">
                                        Facilitator : {{ $speaker->name }}
                                    </span>
                                @endforeach
                            @endif
                            <span
                                class="px-4 py-2 bg-primary rounded-full text-white text-xs font-semibold">{{ $completedFeaturedEvent->eventCategory->title }}</span>
                        </div>
                    </header>

                    <div class="text-base mt-4 text-greish max-w-lg line-clamp-4">
                        {!! $completedFeaturedEvent->description !!}
                    </div>
                    ...
                    <div class="flex flex-col space-y-2 mt-4">
                        <div class="flex space-x-4 ">
                            ...
                            <span class="font-medium text-greish">{{ $completedFeaturedEvent->venue }},
                                {{ $completedFeaturedEvent->venue_address }}</span>
                        </div>
                        ...
                        <div class="flex space-x-4">
                            ...
                            <div class="flex flex-col">
                                <span
                                    class="font-medium text-medium text-greish">{{ $completedFeaturedEvent->event_date }}</span>
                                <span class="text-xs text-greish">{{ $completedFeaturedEvent->event_time }} - Onwards</span>
                            </div>
                        </div>
                        ...
                        <div class="flex space-x-4">
                            ...
                            @if($completedFeaturedEvent->event_fee)
                            <span class="font-medium text-medium text-greish">Rs. {{ $completedFeaturedEvent->event_fee }}</span>
                            @else
                            <span class="font-medium text-medium text-greish">Free</span>
                            @endif
                        </div>
                    ...
                    <footer class="mt-4 flex flex-col sm:flex-row sm:justify-between">
                        <button class="outlined-button">
                            <a href="{{ route('events.show', $completedFeaturedEvent->slug) }}" class="px-20">Read More</a>
                        </button>
                    </footer>
                </div>
            </div>
    ...
    <div class="mx-auto max-w-xs mt-2 sm:mt-10">
        <div class="flex items-center justify-center cursor-pointer">
            <a class="flex space-x-4 text-greish items-center font-medium hover:text-gray-800"
                href="{{ route('events.index') }}">View more
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
            ...
            <div class="grid sm:grid-cols-2 lg:gap-8">
                        ...
                        <div class="relative py-6 px-5">
                            <div class="relative h-96 w-full border-4 rounded-r-2xl border-primary">
                                <div class="absolute mt-4 mr-6 top-4 -left-10 ">
                                    <img src="{{ $featuredBlog->thumbnailUrl() }}" alt="Blog's thumbnail"
                                        class="rounded-r-2xl rounded-b-2xl" />
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="mx-auto mt-6 md:mt-10">
                    <header class="lg:mt-20">
                        ...
                        <span
                            class="px-4 py-2 bg-primary rounded-full text-white text-xs font-semibold">{{ $featuredBlog->blogCategory->title }}</span>
                    </header>
    ...
    <section class="mt-10 sm:mt-24  w-full lg:h-96 overflow-hidden items-center flex justify-center">
        ...
        <div x-data="{ isHovered: false }" class="absolute border-4 border-white rounded-full">
            <button x-on:mouseover="isHovered = true" x-on:mouseleave="isHovered = false"
                class="flex justify-between items-center space-x-2 py-3 px-6 text-center text-white font-extrabold text-xl lg:text-6xl hover:pr-8 ease-out duration-150">
                <a href="{{ route('merchandise.index') }}" class="uppercase">Check out our merch</a>
                <svg x-show="isHovered" x-cloak xmlns="http://www.w3.org/2000/svg" fill="none"
                    viewBox="0 0 24 24" stroke-width="5" stroke="currentColor"
                    class="w-12 h-12 text-white ease-in duration-300">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
                </svg>
                ...
                @foreach ($testimonials as $attestant)
                    <article
                        ...
                        <div x-data="{ readMore: false }" class="mt-6 font-dm-sans text-base">
                            <img src="{{ asset('assets/quote-icon.png') }}"
                                class="absolute w-11 h-8 top-36 lg:top-40 lg:w-28 lg:h-20">
                            <div x-cloak class="text-left text-greish lg:mt-12"
                                :class="readMore ? '' : 'line-clamp-3'">
                                {!! $attestant->testimonial !!}
                            </div>
                            <p x-on:click="readMore = true" x-show="!readMore" class="cursor-pointer">Read More...
                            </p>
                        ...
                        <header class="font-dm-sans">
                            <div class="mt-6">
                                <h2 class="text-primary font-semibold text-2xl text-center">{{ $attestant->name }}
                                </h2>
