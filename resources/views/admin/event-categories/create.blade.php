<x-admin.app title="Event Categories | Create">
                ...
                <div class="w-full p-4 bg-white rounded-lg">
                    <div class="p-4 space-y-6" x-data="{ title:'' }">
                        <x-form.group label="Category Title" for="title">
                            <x-form.input x-model="title" type="text" id="title" name="title" value="{{ old('title') ?? $eventCategory->title }}" />
                            <x-form.error name="title" />
                        </x-form.group>

                        <x-form.group label="Slug" for="slug">
                            <x-form.input x-slug="title" type="text" id="slug" name="slug" value="{{ old('slug') ?? $eventCategory->slug }}" />
